...
static BSTR return_error(const char* msg) {
    ...
    return char_to_bstr(error_json);
}
...
static Config* load_config() {
    char dll_path[MAX_PATH], config_path[MAX_PATH], dll_dir[MAX_PATH];
    if (GetModuleFileNameA(hModule_this_dll, dll_path, MAX_PATH) == 0) {
        fprintf(stderr, "ERROR: Failed to get DLL module path.\n");
        return NULL;
    }
    strncpy(dll_dir, dll_path, MAX_PATH);
    PathRemoveFileSpecA(dll_dir);
    snprintf(config_path, MAX_PATH, "%s\\libnfe.cfg", dll_dir);
    
    FILE* file = fopen(config_path, "r");
    if (!file) {
        fprintf(stderr, "ERROR: Failed to open libnfe.cfg.\n");
        return NULL;
    }
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        if (key && value) {
            ...
            #define SET_CONFIG_PATH(cfg_key, field) if (_stricmp(key, cfg_key) == 0) { \
                ...
                if (PathIsRelativeA(value)) { snprintf(full_path, MAX_PATH, "%s\\%s", dll_dir, value); config->field = _strdup(full_path); } \
...
static BSTR nfe_service_request(const char* service_url, const Config* config, const char* user_payload) {
    ...
    if (!pfx_file) {
        ...
        WSACleanup();
        return return_error("Failed to open client PFX file specified in libnfe.cfg");
    }
    ...
    if (!pfx) {
        ...
        SSL_CTX_free(ssl_ctx);
        WSACleanup();
        return return_error("Failed to parse client PFX file. Is it a valid PKCS12 file?");
    }
    ...
    if (!PKCS12_parse(pfx, config->certificate_pass, &pkey, &cert, NULL)) {
        ...
        SSL_CTX_free(ssl_ctx);
        WSACleanup();
        return return_error("Failed to parse PFX, check certificate_pass in libnfe.cfg.");
    }
    PKCS12_free(pfx);
    ...
    return char_to_bstr(body ? body + 4 : "");
}
